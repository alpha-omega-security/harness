...
func TestResolveSELinuxRelabel(t *testing.T) {
	...
	// "auto" and the empty default must track host detection exactly, so the
	// fix turns on only where it's needed and stays invisible elsewhere.
...
func TestVerifySELinuxMount_NoopCases(t *testing.T) {
	...
	if err := VerifySELinuxMount(context.Background(), Runtime{}, "img:latest", false); err != nil {
		t.Errorf("VerifySELinuxMount(relabel=false) = %v, want nil (no-op)", err)
	}
	// relabel=true but an empty image short-circuits before any exec, so it
	// stays a no-op regardless of host runtime availability.
	if err := VerifySELinuxMount(context.Background(), Runtime{}, "", true); err != nil {
