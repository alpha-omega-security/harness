...
// a container can call a loopback-bound host service. Apple's container
// runtime uses a gateway IP instead of this alias; callers pass that host
...
// DefaultAllow covers common source forges, package registries, and security
// advisory services. Callers should append their harness's model API hosts.
// Entries are matched case-insensitively with the port removed. A leading *.
// matches subdomains.
var DefaultAllow = []string{
	// Service on the container host.
	HostGatewayAlias,
	...
	// index is metadata-only; swiftly reads the toolchain catalogue from
	// www.swift.org and fetches the selected archive from download.swift.org.
...
// Proxy is a small forward proxy the container runner points
...
func (p *Proxy) writeIPDenied(w http.ResponseWriter, method, host string, err error) bool {
...
// Start listens on all interfaces on an ephemeral port and
...
// NewToken returns 32 hex chars of crypto/rand for Proxy-Authorization.
...
// ProxyURL builds the http_proxy-style URL for containers reaching the
// in-process host proxy. Docker/Podman pass HostGatewayAlias; Apple's
// container runtime passes the resolved gateway IP.
func ProxyURL(token, host string, port int) string {
	return fmt.Sprintf("http://harness:%s@%s:%d", token, host, port)
}

// EndpointURL builds the http_proxy-style URL for a proxy reachable at
...
func EndpointURL(token, endpoint string) string {
	return fmt.Sprintf("http://harness:%s@%s", token, endpoint)
}
...
func (p *Proxy) dialTarget(host, port string) string {
	if p.isAPIHost(host) {
...
func (p *Proxy) gatewayDialHost() string {
...
// Serve runs p on addr and blocks until the server stops. The egress
...
// ListenFirstIface is a listen-host keyword callers can use to request the
// IPv4 address of the first non-loopback interface.
const ListenFirstIface = "first-iface"
